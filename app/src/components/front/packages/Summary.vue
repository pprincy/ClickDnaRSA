<template>
  <v-col cols="12" sm="12" xs="12" md="12">
    <v-row>
      ...
      <v-col cols="12" md="4" sm="4" xs="12">
        <v-card
          @click="changeStatus('done,planning,planned,post,in_transit,draft')"
          :loading="loader"
        >
          <v-card-title class="headline">
            Success
      ...
      <v-col cols="12" md="4" sm="4" xs="12">
        <v-card @click="changeStatus('exception')" :loading="loader">
          <v-card-title class="headline">
            Failed
            <v-spacer></v-spacer>
...
export default {
  ...
  created() {
    this.fetchPackageDataSummary();
    EventBus.$off("packCountUpdate");
    EventBus.$on("packCountUpdate", () => {
      this.fetchPackageDataSummary();
    });
  },
  methods: {
    ...mapActions(["getPackageDataSummary"]),
    changeStatus(status) {
      EventBus.$emit("packageStatus", status);
    },
    fetchPackageDataSummary() {
      this.loader = true;
      this.getPackageDataSummary()
        .then((res) => {
          this.loader = false;
